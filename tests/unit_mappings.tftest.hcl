...
run "rejects_content_and_generation" {
  ...
  variables {
    secrets = {
      s1 = { content = "a", generation = { generation_type = "BYTES", generation_template = "T" } }
    }
  }
  expect_failures = [var.secrets]
}
...
run "rejects_passphrase_length_on_non_passphrase" {
  ...
  variables {
    secrets = {
      s1 = { generation = { generation_type = "SSH_KEY", generation_template = "T", passphrase_length = 20 } }
    }
  }
  expect_failures = [var.secrets]
}
...
run "rejects_bad_generation_type" {
  command = plan
  variables {
    secrets = { s1 = { generation = { generation_type = "RSA_KEY", generation_template = "T" } } }
  }
  expect_failures = [var.secrets]
}
