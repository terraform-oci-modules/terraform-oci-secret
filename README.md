...
`generation` maps to `secret_generation_context`: `generation_type` is
`PASSPHRASE`, `SSH_KEY`, or `BYTES`; `generation_template` names a predefined
template (e.g. `SECRETS_DEFAULT_PASSPHRASE`, `SECRETS_DEFAULT_SSH_KEY_PEM`).
